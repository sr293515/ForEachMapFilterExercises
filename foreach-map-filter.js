...
function doubleValues(arr){
    newArray = [];
    arr.forEach(function(doubled){
        newArray.push(doubled * 2)
    })
    // for(i = 0; i < arr.length; i++){  // For loop version
...
function showFirstAndLast(arr){
    ...
    arr.forEach(function(firstLast){
        ...
        let last = firstLast[firstLast.length -1];
        let concat = first.concat(last);
        firstAndLast.push(concat);
    }); 
    return firstAndLast;
};
...
function vowelCount(str){
    const lowerCasedString = str.toLowerCase(); // LowerCase all the letters in the string
    const separatedString = lowerCasedString.split(''); // Example: e, l, i, e
    const countedVowels = {};   // Where the keys and the values are going to be stored.
    const vowels = 'aeiou';
    separatedString.forEach(function(letter){ // Letter by letter
        if(vowels.indexOf(letter) !== -1){  // if vowels is not equal to negative 1, in other words if there are vowels included.
            if(letter in countedVowels){    // 
...
function doubleValuesWithMap(arr) {
    let doubledArray = arr.map(doubleFunction)
...
// function doubleValuesWithMap(arr) {  // Solution Way.
...
function extractKey(arr, key){
    return arr.map(function(reveal){
        return reveal[key];
    });
};
...
function find(arr, searchValue) {
    return arr.filter(function(unFiltered){
        return unFiltered === searchValue;
    })[0]; // So it stops at the first result;
};
